...
#   - Codex (OpenAI): .codex/agents/ symlink + AGENTS.md (native)
...
setup_claude() {
  ...
  echo -e "${GREEN}  ✓ .claude/skills -> agents/${NC}"
...
setup_gemini() {
  ...
  ln -s "$SKILLS_SOURCE" "$target"
  echo -e "${GREEN}  ✓ .gemini/skills -> agents/${NC}"
...
setup_codex() {
  ...
  ln -s "$SKILLS_SOURCE" "$target"
  echo -e "${GREEN}  ✓ .codex/skills -> agents/${NC}"
